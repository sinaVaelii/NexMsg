...
//
// ChatService provides real-time messaging over gRPC.
type ChatServiceClient interface {
	// Stream opens a bidirectional stream for real-time chat.
...
func (c *chatServiceClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ClientEvent, ServerEvent], error) {
...
type ChatService_StreamClient = grpc.BidiStreamingClient[ClientEvent, ServerEvent]
...
func (UnimplementedChatServiceServer) Stream(grpc.BidiStreamingServer[ClientEvent, ServerEvent]) error {
...
func _ChatService_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(ChatServiceServer).Stream(&grpc.GenericServerStream[ClientEvent, ServerEvent]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ChatService_StreamServer = grpc.BidiStreamingServer[ClientEvent, ServerEvent]
